...
KEY_BODY_NAMES = []#"right_hand", "left_hand",]
...
for i in range(10):
    ...
    asset_file = "mjcf/common_rig_strongrpy.xml"
    ...
    humanoid_asset = gym.load_mjcf(sim, asset_root, asset_file, asset_options)
    actor_handles.append(gym.create_actor(env, humanoid_asset, gymapi.Transform(p=gymapi.Vec3(0., 0.0, 0.)), name, 0, 0))
# look at the first env
cam_pos = gymapi.Vec3(12, 3, 2)
cam_target = gymapi.Vec3(0, 3, 1)
...
motion_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../assets/amp/motions/VAAI3.npy")
sampled_motion_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../assets/amp/motions/VAAI_GRP2.npy")
# motion_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../assets/amp/motions/rpy.npy")
# motion_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../assets/amp/motions/amp_humanoid_dance.npy")
...
_dof_pos = _dof_state.view(10, num_dof, 2)[..., 0]
_dof_vel = _dof_state.view(10, num_dof, 2)[..., 1]
actor_indices = torch.arange(10, dtype=torch.long, device=device)#.view(10, 1)
...
while not gym.query_viewer_has_closed(viewer):
    for i in range(10):
        ...
        _root_states[i, 0:3] = root_pos
        _root_states[i, 3:7] = root_rot
        _root_states[i, 7:10] = root_vel
        _root_states[i, 10:13] = root_ang_vel
        # dof_pos[0, 4] = 1.5629
        # dof_pos[0, 5] = -0.2429
        # dof_pos[0, 6] = 0.0451
        # dof_pos[0, 7] = 0.5138
        ...
        # dof_pos[0, -1] = 1.5
        _dof_pos[i] = dof_pos[0]
        _dof_vel[i] = dof_vel[0]

    # Added from JTM
    env_ids_int32 = actor_indices.to(dtype=torch.int32)
